...
class Confirming extends StatefulWidget {
  ...
  const Confirming({super.key, required this.place_name, required this.place_id, required this.place_lat, required this.place_lng});
...
class _ConfirmingState extends State<Confirming> {
  ...
  void initState() {
    ...
    // _addMarker(LatLng(widget.place_lat, widget.place_lng), "destination", redMarkerIcon);
    addCustomIcon();
    circles = {
      Circle(
  ...
  @override
  void dispose() {
    developer.log("canceled", name: "confirmDisposed");
  ...
  void addCustomIcon() {
    ...
    BitmapDescriptor.fromAssetImage(const ImageConfiguration(), "assets/redMarker.png").then(
      (icon) {
        setState(() {
          redMarkerIcon = icon;
        });
  ...
  _getPolyline() async {
    PolylineResult result = await polylinePoints.getRouteBetweenCoordinates(
        googleAPiKey, PointLatLng(_currentLocation!.latitude!, _currentLocation!.longitude!), PointLatLng(widget.place_lat, widget.place_lng),
  ...
  void getCurrentLocation() async {
    ...
    locationSubscription = location.onLocationChanged.listen(
      (newLoc) {
        if (_tracking) {
          developer.log("location updated!", name: 'confirmLocation');
          _currentLocation = newLoc;
          updatedGPS(newLoc);
          setState(() {});
          if (Geolocator.distanceBetween(_currentLocation!.latitude!, _currentLocation!.longitude!, widget.place_lat, widget.place_lng) < 500) {
  ...
  updatedGPS(LocationData location) {
    ...
    double calculatedRotation = Geolocator.bearingBetween(location.latitude!, location.longitude!, widget.place_lat, widget.place_lng);
