...
class _BusConfirmState extends State<BusConfirm> {
  ...
  @override
  void dispose() {
    developer.log("disposed", name: "BusConfirm");
  ...
  Future insertFavPlace(Place place) async {
    await PlacesDatabase.instance.create(favTable, place);
    readFavPlaces();
  }

  Future insertHisPlace(Place place) async {
    await PlacesDatabase.instance.create(hisTable, place);
  }
  ...
  Future<LatLng> _findNearbyStop(List stops) async {
    ...
    var minDistance = Geolocator.distanceBetween(_currentLocation!.latitude!, _currentLocation!.longitude!, _centerLatLong.latitude, _centerLatLong.longitude);
    developer.log(minDistance.toString(), name: 'min distance');

    var index = 1;
    for (var stop in stops) {
      _addStopMarker(LatLng(stop['StopPosition']['PositionLat'], stop['StopPosition']['PositionLon']), stop['StopName']['Zh_tw'], index);

      var distance = Geolocator.distanceBetween(
          _currentLocation!.latitude!, _currentLocation!.longitude!, stop['StopPosition']['PositionLat'], stop['StopPosition']['PositionLon']);
  ...
  _getPolyline() async {
    PolylineResult result = await polylinePoints.getRouteBetweenCoordinates(
        googleAPiKey, PointLatLng(_currentLocation!.latitude!, _currentLocation!.longitude!), PointLatLng(_centerLatLong.latitude, _centerLatLong.longitude),
  ...
  getCurrentLocation() async {
    ...
    locationSubscription = location.onLocationChanged.listen(
      (newLoc) {
        developer.log("location listening!", name: 'location');

        if (_tracking) {
          developer.log("location updated!", name: 'location');
          _currentLocation = newLoc;
          if (Geolocator.distanceBetween(_currentLocation!.latitude!, _currentLocation!.longitude!, _centerLatLong.latitude, _centerLatLong.longitude) < 500) {
            Navigator.of(context).pushReplacement(FadePageRoute(Arrived(
              placeName: _centerName,
            )));
          }
          updatedGPS(newLoc);
        } else {
          developer.log("not tracking", name: 'location');
        }
      },
    );
  }
  ...
  updatedGPS(LocationData location) {
    ...
    double calculatedRotation = Geolocator.bearingBetween(location.latitude!, location.longitude!, _centerLatLong.latitude, _centerLatLong.longitude);
  ...
  startTracking() async {
    if (Geolocator.distanceBetween(_currentLocation!.latitude!, _currentLocation!.longitude!, _centerLatLong.latitude, _centerLatLong.longitude) > 500) {
      ...
      insertHisPlace(Place(id: _centerID, name: _centerName, lat: _centerLatLong.latitude, lng: _centerLatLong.longitude));
                                        ...
                                        child: InkWell(
                                          ...
                                          child: Padding(
                                            ...
                                            child: Icon(
                                              ...
                                              color: _faved ? Colors.red[700] : Theme.of(context).colorScheme.onSecondaryContainer,
                                              size: 24.0,
                                            ),
                                          ),
                                          onTap: () => favOnTap(),
